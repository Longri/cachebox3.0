		...
		if (Database.Data.MasterDatabaseId > 0) {
			if (oldSolverCheckSum != newSolverCheckSum) {
				if (oldSolverCheckSum != newSolverCheckSum) {
					...
					// Text
					int dbCheckSum = -1;
					SQLiteGdxDatabaseCursor c = Database.Data.rawQuery("select SolverCheckSum from Replication where CacheId=? and ChangeType=?", new String[] { String.valueOf(CacheId), String.valueOf(ChangeType.SolverText.ordinal()) });
					c.moveToFirst();
					while (!c.isAfterLast()) {
		...
		if (Database.Data.MasterDatabaseId > 0) {
			if (oldNoteCheckSum != newNoteCheckSum) {
				if (oldNoteCheckSum != newNoteCheckSum) {
					...
					// Text
					int dbCheckSum = -1;
					SQLiteGdxDatabaseCursor c = Database.Data.rawQuery("select NotesCheckSum from Replication where CacheId=? and ChangeType=?", new String[] { String.valueOf(CacheId), String.valueOf(ChangeType.NotesText.ordinal()) });
					c.moveToFirst();
					while (!c.isAfterLast()) {
						dbCheckSum = c.getInt(0);
						break;
	...
	private static void Changed(long CacheId, int oldCheckSum, int newCheckSum, String checkSumType, ChangeType changeType, String WpGcCode) {
		if (Database.Data.MasterDatabaseId > 0) {
			if (oldCheckSum != newCheckSum) {
				...
				int dbCheckSum = -1;
				SQLiteGdxDatabaseCursor c = Database.Data.rawQuery("select " + checkSumType + " from Replication where CacheId=? and ChangeType=? and WpGcCode=?", new String[] { String.valueOf(CacheId), String.valueOf(changeType.ordinal()), WpGcCode });
				c.moveToFirst();
				while (!c.isAfterLast()) {
					dbCheckSum = c.getInt(0);
					break;
