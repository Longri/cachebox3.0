...
public abstract class ClusterLayer<ClusterablePoint> extends Layer {
    ...
    public ClusterLayer(Map map, ClusterRendererFactory clusterRendererFactory) {
        super(map);

        mClusterRenderer = clusterRendererFactory.create(this);
        mRenderer = mClusterRenderer;
    }
    ...
    public final void populate() {
        mClusterRenderer.populate(size());
    }

    /**
     * TODO
     * If the given Item is found in the overlay, force it to be the current
     * focus-bearer. Any registered {link ItemizedClusterLayer#OnFocusChangeListener}
