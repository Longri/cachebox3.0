...
public class ExtAudioRecorder {
    ...
    /**
     * Default constructor Instantiates a new recorder, in case of compressed recording the parameters can be left as 0. In case of errors,
    ...
    public static ExtAudioRecorder getInstance(Boolean recordingCompressed) {
        ...
        if (recordingCompressed) {
            result = new ExtAudioRecorder(false, MediaRecorder.AudioSource.MIC, sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 0;
            do {
                result = new ExtAudioRecorder(true, MediaRecorder.AudioSource.MIC, sampleRates[i], AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);
    ...
    /**
     * Prepares the recorder for recording, in case the recorder is not in the INITIALIZING state and the file path was not set the recorder
            ...
            if (state == State.INITIALIZING) {
                if (rUncompressed) {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
                        ...
                        randomAccessWriter.setLength(0); // Set file length to 0, to prevent unexpected behavior in case the file already
                        // existed
                        ...
                        randomAccessWriter.writeShort(Short.reverseBytes((short) (nChannels * bSamples / 8))); // Block align,
    ...
    public void release() {
        if (state == State.RECORDING) {
            ...
            if ((state == State.READY) & (rUncompressed)) {
                ...
                (new FileHandle(filePath)).delete();
            }
        }
